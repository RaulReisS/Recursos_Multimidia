...
import br.com.raulreis.recursosmultimdia.LocalizacaoMapsActivity.Companion.REQUEST_ERROR_PLAY_SERVICES
import com.google.android.gms.common.ConnectionResult
import com.google.android.gms.common.GoogleApiAvailability

class LocalizacaoMapsActivity : AppCompatActivity() {
    private val viewModel: MapViewModel by lazy {
        ViewModelProviders.of(this).get(MapViewModel::class.java)
    }
    private val fragment: AppMapFragment by lazy {
        supportFragmentManager.findFragmentById(R.id.map) as AppMapFragment
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_localizacao_maps)
    }
    ...
    private fun handleLocationError(error: MapViewModel.LocationError?) {
        if (error != null) {
            when (error) {
                is MapViewModel.LocationError.ErrorLocationUnavaible ->
        ...
        if (result.hasResolution()) {
            try {
                result.startResolutionForResult(
                    this, LocalizacaoMapsActivity.REQUEST_ERROR_PLAY_SERVICES)
